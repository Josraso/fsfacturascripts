...
class FsFacturaScripts extends Module
{
                ...
                'input' => [
                    ...
                    [
                        ...
                        'html_content' => '<div class="alert alert-info">
                            ...
                            <code>*/10 * * * * wget -q -O- "https://' . Tools::getHttpHost(false, false) . __PS_BASE_URI__ . 'modules/fsfacturascripts/cron.php?token=' . Configuration::get('FS_CRON_TOKEN') . '" > /dev/null 2>&1</code><br>
    ...
    private function sendWebhookToFacturaScripts($order)
    {
        try {
            ...
            // Usar siempre la fecha del pedido para evitar problemas de compatibilidad
            // con diferentes versiones de PrestaShop que pueden tener esquemas distintos
            $state_date = $order->date_add;

            PrestaShopLogger::addLog(
                'FacturaScripts DEBUG: Usando fecha del pedido: ' . $state_date,
                1,
                null,
                'Order',
                $order->id,
